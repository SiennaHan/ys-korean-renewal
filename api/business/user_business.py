...
#
# **예외가 하나 있다 — `ko_signup_code_use` 는 여기 없는 것이 맞다.** 게스트는
# 기관 코드로 가입할 수 없으므로 게스트 id 로 된 행이 애초에 없다. 지우는 쪽에는
# 있다(`ANONYMIZE_MODELS`, 2026-08-28). 목록을 맞추려고 여기 더하지 마라 —
# 그 표의 `user_id` 만 Integer 라 게스트 id 문자열을 넣으면 MySQL 이 조용히
# 0 으로 바꾼다.
