<!-- 관찰: app/src/styles/tokens.css, app/src/styles/activity.css, app/src/styles/nav.css, app/src/styles/auth.css, app/src/styles/globals.css, app/src/components/main/activity, app/src/components/ui, app/src/shared/constants @ bdf6294
     — 확인: 2026-09-11 · **미션대화의 상황 그림 둘을 16:9 로 맞췄다**(기획 요청).
       ...
       **왜 납작했나** — 원본이 전부 2400×1350(16:9)이고 칸 폭이 307px 이라 폭에
       ...
       원본보다 납작했다. 브리핑도 같은 꼴이었다 — 칸 319px 에 높이 132px 고정이라
       2.42:1 이었다. 이제 각각 **307×173 · 319×179** 고 둘 다 1.78 이다
       (브라우저에서 계산값을 재서 확인). 원본과 같은 비율이라 **잘려 나가는 데가 없다.**
     ...
     — 확인: 2026-09-09 · 그 배너가 **시안 승인분으로 세 줄이 됐다**(`.event-banner-after`
       ...
       **이 문서가 캐는 두 축(리터럴 색 · 비4배수 간격)은 해당 없다** — CSS 도 새
       ...
       `activity.css` 줄 수도 그대로다. `check:css` 와 `parity:activity` 를 돌려 0 을 봤다.
       앞 확인: 2026-09-03 · 미션대화 발화 피드백을 붙였다(`8f059d2`). 이 문서가 캐는 두 축
...
|---|---|---|---|---|
| **학습 활동** | `styles/activity.css` (690줄) | `.activity-frame .*` (목업 이름 그대로) | semantic 토큰 | ○ |
