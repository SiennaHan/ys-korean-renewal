...
const HangulTracingCanvas = forwardRef<HangulTracingCanvasHandle, Props>(
	({ char, isWriteDone }: Props, ref) => {
		const canvasRef = useRef<HTMLCanvasElement>(null);
		const [inputChar, setInputChar] = useState(char);
		const [statusText, setStatusText] = useState(
			'연습할 글자를 입력하고 "연습 시작" 버튼을 누르세요.',
		);
		...
		const generateWaypointsFromStroke = (
			strokePoints: Point[],
			pointsPerSegment = 50,
		): Point[] => {
			let waypoints: Point[] = [];
			...
			for (let i = 0; i < strokePoints.length - 1; i++) {
				...
				const segmentWaypoints = generateLineWaypoints(
					start,
					end,
					pointsPerSegment,
				);
				waypoints = waypoints.concat(
					i > 0 ? segmentWaypoints.slice(1) : segmentWaypoints,
				);
			}
			return waypoints;
		};
		...
		const setupChallengesForChar = (char: string) => {
			...
			if (VOWEL_TYPES.VERTICAL.includes(ju)) {
				...
				const actualChoseongBoxWidth =
					leftSectionWidth * choseongActualSizeFactor;
				...
				if (hasJongseong) {
					...
					boxes.choseong = {
						x: x + choseongXOffset,
						y,
						width: actualChoseongBoxWidth,
						height: choseongBoxH,
					};
					boxes.jungseong_horizontal = {
						x,
						y: juHY,
						width: leftSectionWidth,
						height: juHH,
					};
					boxes.jungseong_vertical = {
						x: juVXStart,
						y,
						width: juVBoxWidth,
						height: unitH * 2,
			...
			if (VOWEL_TYPES.COMPOSITE.includes(ju)) {
				...
				if (juData?.vowel_horizontal) {
					jamoList.push({
						jamo: ju,
						type: "vowel_h",
						box: boxes.jungseong_horizontal,
					});
				}
				if (juData?.vowel_vertical) {
					jamoList.push({
						jamo: ju,
						type: "vowel_v",
						box: boxes.jungseong_vertical,
					});
				}
				if (hasJongseong) {
				...
				if (hangulStrokeData[item.jamo]) {
					...
					for (const normalizedStroke of strokes) {
						...
						if ((normalizedStroke as CircleStroke).type === "circle") {
							...
							const radius =
								(Math.min(box.width, box.height) / 100) * circleStroke.r;
							waypoints = generateCircleWaypoints(center, radius, 100);
							waypoints.unshift(waypoints.pop()!);
							waypoints.unshift(waypoints.pop()!);
						} else {
							const realCoordStroke = transformStroke(
								normalizedStroke as Point[],
								item.box,
							);
		...
		const drawAllGuides = () => {
			...
			challenges.forEach((challenge, index) => {
				if (challenge.completed) {
					...
					ctx.strokeStyle =
						index === currentChallengeIndex ? "#aaaaaa" : "#e0e0e0";
					ctx.lineWidth = LINEWIDTH;
					ctx.lineCap = "round";
					ctx.lineJoin = "round";
					ctx.stroke();
					...
					// 시작점 위치 계산
					const dotPosition = challenge.isClosedLoop
						? challenge.waypoints[1]
						: challenge.waypoints[0];
					const startPointColor =
						index === currentChallengeIndex ? "#C489F4" : "#C489F4";
		...
		const undo = useCallback(() => {
			...
			setStatusText(
				`도전 ${previousIndex + 1}: '${char}'의 ${previousIndex + 1}번째 획을 그리세요.`,
			);
		...
		const triggerSuccess = (challenge: Challenge) => {
			...
			if (currentChallengeIndex + 1 >= challenges.length) {
				...
				setStatusText(
					`성공! 다음 획을 그려보세요. (${currentChallengeIndex + 2}/${challenges.length})`,
				);
			}
			setActiveChallenge(null);
		};
		...
		const handleStart = (e: CanvasEvent) => {
			...
			const b = Math.hypot(
				pos.x - startPointCheck.x,
				pos.y - startPointCheck.y,
			);
			const check = b < 20;
		...
		const handleMove = (e: CanvasEvent) => {
			...
			const currentPoint = getCoordinates(e);
			const pos = { x: currentPoint.x, y: currentPoint.y };
			const { index: closestIndex, distance } = findClosestPointOnPath(
				pos,
				activeChallenge.waypoints,
			);
			...
			if (
				activeChallenge.isClosedLoop &&
				progressIndex < pathLength * 0.1 &&
				closestIndex > pathLength * 0.9
			) {
				setIsDrawing(false);
			...
			const isWrappingAround =
				activeChallenge.isClosedLoop && backwardDistance > pathLength * 0.8;
			...
			if (activeChallenge.isClosedLoop) {
				...
				if (
					activeChallenge.state.halfwayReached &&
				...
				const endPoint =
					activeChallenge.waypoints[activeChallenge.waypoints.length - 1];
				const distToEnd = Math.hypot(pos.x - endPoint.x, pos.y - endPoint.y);
		...
		const handleEnd = () => {
			...
			const progressPercentage =
				activeChallenge.state.progressIndex /
				(activeChallenge.waypoints.length - 1);
