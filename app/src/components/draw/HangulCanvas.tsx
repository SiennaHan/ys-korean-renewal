...
export default function HangulCanvas({text, returnImage, onClose}: Props)  {
   ...
   */
  const draw = useCallback((event: CanvasEvent): void => {
    if (!isDrawing) return;
  ...
  return (
    <div className="flex flex-col items-center justify-center font-inter">
      <div className="bg-white rounded-xl transition-all hover:shadow-3xl max-w-sm w-full">
        {/* Signature Canvas Area */}
        <div className='w-full text-center mb-[10px]'>여기에 손가락으로 쓰세요.</div>
        <div 
          className="rounded-lg border-4 border-dashed border-gray-300 bg-white cursor-crosshair shadow-inner mx-auto"
        ...
        {/* Control Buttons */}
        <div className="flex justify-between items-center space-x-3 p-2">
          <button
            onClick={clearCanvas}
            className="bg-white text-[#4396f4] font-bold py-3 px-4 rounded-lg shadow-md hover:bg-gray-100 
											transition duration-300 transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none border-1 cursor-pointer"
          >
            <RotateCcw />
          </button>
          <div className="flex items-center text-white font-bold">
            {isCorrect === true && <div className="rounded-[10px] text-[20px] pl-2 pr-2 bg-green-400">Correct</div>}
            {isCorrect === false && <div className="rounded-[10px] text-[20px] pl-2 pr-2 bg-red-400">Incorrect</div>}
            {isUploading === true && <div className="rounded-[5px] text-[14px] !text-gray-400 !font-normal pl-2 pr-2 bg-gray-white">분석중...</div>}
          </div>
          <button
            onClick={saveSignature}
            className="bg-[#4396f4] text-white font-semibold py-3 px-4 rounded-lg shadow-md hover:bg-blue-500 
											transition duration-300 transform hover:scale-[1.02] active:scale-[0.98] focus:outline-none cursor-pointer
                      disabled:text-gray-300 disabled:shadow-none disabled:opacity-70 disabled:cursor-not-allowed disabled:bg-white disabled:hover:bg-white"
            disabled={!hasLine || isUploading}
          >
            {isUploading ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#4396f4]"></div> : <Upload />}
          </button>
        </div>
