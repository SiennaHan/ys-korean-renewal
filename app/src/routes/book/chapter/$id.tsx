import { BookHeader } from "@/components/ui/book-header";
import { books } from "@/shared/data/book";
import { chapters } from "@/shared/data/chapter";
import { modules } from "@/shared/data/module";
import { units } from "@/shared/data/unit";
import type { ModuleType } from "@/types/book.types";
import { createFileRoute, useNavigate } from "@tanstack/react-router";
import { motion } from "framer-motion";
import { ChevronRight } from "lucide-react";

export const Route = createFileRoute("/book/chapter/$id")({
	component: RouteComponent,
});

const buttonBase =
	"w-full flex mb-[7px] py-[6px] px-[8px] rounded-[5px] bg-[#fff] border-1 border-[#efefef] shadow-[0_0px_6px_0.5px_rgb(94,129,169,0.1)] cursor-pointer hover:bg-[#e9f2fc] active:bg-[#e9f2ff] \
										disabled:text-[#888888] disabled:shadow-none disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-[#ddd] disabled:hover:bg-[#ddd]";

function RouteComponent() {
	const { id } = Route.useParams(); // 여기서 파라미터를 가져옵니다.
	const chapterId = Number(id ?? 0);

	const navigate = useNavigate();

	const chapter = chapters.find((item) => item.id === chapterId);
	const book = books.find((item) => item.id === chapter?.book_id);

	// const unitList = units.find((item) => item.chapter_id === chapterId)
	const unit1 = units.find(
		(item) => item.chapter_id === chapterId && item.seq === 1,
	);
	const unit2 = units.find(
		(item) => item.chapter_id === chapterId && item.seq === 2,
	);

	const moduleList1: ModuleType[] = modules.filter(
		(item) => item.unit_id === unit1?.id,
	);
	const moduleList2: ModuleType[] = modules.filter(
		(item) => item.unit_id === unit2?.id,
	);

	const goProblem = (module: ModuleType) => {
		console.log("moduleCode=>", module.code);
		navigate({
			to: `/book/chapter/unit/${module.scene_type}/${module.code}`,
		});
	};

	const ShowModule = () => {};

	const previousPage = `/book/${book?.id}`;

	return (
		<motion.div
			initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			exit={{ opacity: 0 }}
			transition={{ duration: 1 }}
			className="h-full"
		>
			<div className="h-full bg-[#f8f8f8]">
				<BookHeader title={book?.title} previousPage={previousPage} />
				<div className="flex bg-white">
					<div className="w-[5px] bg-[#037]" />
					<div className="flex h-[53px] items-center">
						<div className="flex w-[40px] justify-center pl-[15px] font-extrabold text-[#037] text-[32px]">
							{chapter?.seq}
						</div>
						<div className="flex flex-col pl-[15px]">
							<div className="text-[14px]">{chapter?.title}</div>
							<div className="text-[#999] text-[10px]">{chapter?.eng}</div>
						</div>
					</div>
				</div>

				<div className="pr-[15px] pl-[15px]">
					<div className="mt-[18px] ml-[10px] font-bold text-[12px]">
						언어 활용 연습
					</div>
					<div className="mt-[10px] gap-[10px]">
						{moduleList1.map((item) => (
							<button
								key={item.id}
								onClick={(e) => goProblem(item)}
								className={buttonBase}
								disabled={item.is_disabled}
							>
								<div className="flex w-[20px] items-center justify-center">
									<ChevronRight />
								</div>
								<div className="ml-[10px]">
									<div className="font-bold text-[14px]">{item.title}</div>
									<div className="text-[#bbb] text-[10px]">{item.eng}</div>
								</div>
							</button>
						))}
					</div>

					<div className="mt-[18px] ml-[10px] font-bold text-[12px]">
						기초 과정
					</div>

					<div className="mt-[5px] gap-[15px]">
						{moduleList2.map((item) => (
							<button
								key={item.id}
								onClick={(e) => goProblem(item)}
								className={buttonBase}
								disabled={item.is_disabled}
							>
								<div className="flex w-[20px] items-center justify-center">
									<ChevronRight />
								</div>
								<div className="ml-[10px]">
									<div className="font-bold text-[14px]">{item.title}</div>
									<div className="text-[#bbb] text-[10px]">{item.eng}</div>
								</div>
							</button>
						))}
					</div>
				</div>
			</div>
		</motion.div>
	);
}
