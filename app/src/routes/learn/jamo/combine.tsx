...
export const Route = createFileRoute("/learn/jamo/combine")({
...
function RouteComponent() {
	...
	return (
		<div className="flex h-full flex-col bg-[#F6F7F8]">
			...
			{stage === "select" ? (
				<div>
					<div className="bg-white px-[16px] pb-[40px]">
					...
					<div className="px-[16px]">
						<div className="mt-[24px] flex w-full items-center gap-[5px] text-[14px]">
							...
							<div className="font-bold text-[#0180FF]">맨 앞에 오는 소리</div>
						...
						<div className="mt-[12px] w-full">
							<div className="grid grid-cols-6 gap-x-[5px] gap-y-[12px]">
				...
				<div className="bg-white px-[16px]">
					...
					<div className="mt-[16px] flex justify-center gap-[8px]">
						<button
							...
							className={clsx(baseControlButton, "bg-[#FFE8E8] text-[#F15F49]")}
							disabled={isSucceed}
						>
							Erase all
						</button>
						<button
							onClick={undoDrawing}
							className={clsx(baseControlButton, "bg-[#F6F7F8] text-[#383A3F]")}
							disabled={isSucceed}
						>
							Undo
						</button>
			...
			<div
				className={`flex w-full items-end justify-center p-[16px] bg-[${stage === "select" ? "#F6F7F8" : "#fff"}]`}
			>
				<audio className="hidden" src={audioSrc} ref={audioRef}>
					Your device does not support the audio.
				</audio>
				{isExit ? (
					<button onClick={exit} className={clsx(baseButton, "!bg-green-500")}>
						완료 done
					</button>
				) : (
					<button
