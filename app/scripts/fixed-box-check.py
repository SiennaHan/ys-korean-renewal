...
`t("키")` 만 넣는 자리는 뺀다 — 사람이 언어마다 보고 정한 글자다. **`t("키", {...})`
는 빼지 않는다** — 그 안으로 변수가 들어가므로 위험이 하드코딩과 같다.
...
def main() -> int:
	...
	for tsx in sorted((ROOT / "src").rglob("*.tsx")):
		...
		for i, line in enumerate(lines):
			...
			# 로케일 문구만 넣는 t() 는 건너뛴다 — 사람이 언어마다 보고 정한 글자다.
			# **다만 `t(키, { ... })` 는 건너뛰지 않는다** — 그 안에 변수가 들어가고,
			# 그러면 칸이 넘칠 위험은 하드코딩과 똑같다. 2026-08-27 에 서울 퍼즐의
			# `{locName} 완료!` 를 t() 로 옮기자 이 검사가 그 자리를 놓쳤다.
			if inner.startswith("/*"):
				continue
			if re.match(r'^t\(\s*[\'"`][^\'"`]+[\'"`]\s*\)', inner):
				continue
			key = f"{rel} .{used[0]}"
