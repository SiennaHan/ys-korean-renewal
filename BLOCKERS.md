...
`글로벌_교재기반_콘텐츠_v26.xlsx` 의 `n8_jamo`, 컬럼 12→27.
...
| 무엇 | 원장 v26 (새 책) | 목업 `screens_uiux`(홈·목록 절) | 앱 | 처리 |
|---|---|---|---|---|
...
**원장 정본은 이제 `글로벌_교재기반_콘텐츠_v26.xlsx` 다.**
...
| **v26** | **529행** | **정본.** v25 에 사라진 400행을 이었다 |
...
| | 몇 행 | 무엇 |
|---|---|---|
| 앞 따옴표가 먹힌 것 | **57** | JSON `'유리'는 받침이 없어요` / v26 `유리'는 받침이 없어요` |
| 내용까지 다른 것 | **6** | `id` 4 · 25 · 37 · 54 · 264 · 509. 받침 설명이 다른 낱말을 가리키던 것과 `짓다/지으셨다` 정정 |

**지금 `build-content.py` 를 돌리면 이 63행이 앱으로 되돌아간다.** 원장이 정본이라는
규칙이 이번만은 반대로 작동한다 — 원장을 먼저 고치고 나서 돌려야 한다.
되살릴 값은 `app/src/shared/data/n4_blank_question.json` 의 `grammar_focus_revised` 다.
...
`dev_spec_v1` §16 은 정정판 문장을 **학습자가 바로 보는 것**이라 적었다. 화면은 그렇지
않다. 한 줄 바꾸면 되는 일이지만 **정정판 63행이 원장에서 되돌아간 상태**라
위쪽을 먼저 정리해야 한다. 순서 — 원장 63행 복구 → 생성 → 화면을 정정판으로.

### `n8_자모_미보유` 시트가 v26 과 어긋난다

그 시트는 활동 27개를 **"문제 없음"** 이라 적는다(`YK0012`~`YK0040`). v25 의 구글
시트 가져오기에는 맞는 말이었지만 **v26 의 `n8_jamo` 에는 그 27개가 다 있다** —
`problem.ts` 에서 되살린 400행이 바로 그것이다. 지금 그 시트를 믿으면 있는 콘텐츠를
없다고 판단한다. 지우거나 "v25 시점" 이라고 적어야 한다.
...
| 무엇 | 상태 |
|---|---|
| `word_refs` 80행 | v25 가 129행 중 80행만 채웠고 값은 `content` 와 100% 같다. 어느 행에 채우는지가 시트 조인의 흔적이라 파생되지 않는다. **v25 값은 이어받고 새 400행은 비워 두었다.** 전부 채울지 비울지, 아니면 이 칸 자체가 오매핑인지 정해야 한다 |
...
**재발 방지** — `illust/build_v20~v26.py` 일곱 개 전부에 대상 존재 확인을 넣었다.
...
| `n8_jamo` | **포팅 완료**(v26 · 529행). 전부 `draft` — 검수 대기. 한 번 잃고 다시 만들었다 — §2-b |
