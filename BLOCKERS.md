...
**목업 갱신이 필요하다.** `phase1/nav_mockup_uiux.html` 의 자모 화면이 구 교재
기준이고, 캡처(`app/src/mockups/nav__jamo__resume.html`)도 그것에서 떴다.
문구 셋을 새 책으로 고치고 다시 캡처하면 이 화면이 목업 대조에 들어간다 —
`app/scripts/activity-parity.tsx` 에 한 줄(`SCREENS.nav__jamo__resume = <Jamo />`)이면
되고, 그 화면이 정적으로 그려진다는 것은 확인해 두었다(3,116자).

이 저장소는 **구현이 목업과 다르면 목업이 기준**이다. 다만 그 규칙은 시각
(레이아웃·치수·색)에 대한 것이고, 여기 어긋남은 **콘텐츠**(활동 이름·묶음명)다.
콘텐츠 정본은 원장이므로 앱을 되돌리지 않았다.
