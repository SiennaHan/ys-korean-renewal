...
**검수할 것 하나가 목업 대조에서 나왔다 (2026-08-21).**
자모 목록 화면을 목업 대조에 넣어 보니 **모음1 의 낱자 수가 세 곳에서 갈렸다.**

| 어디 | 모음1 | ㅐ·ㅔ |
|---|---|---|
| 원장 `v24` `n8_jamo` | **10자** — 아·애·어·에·오·외·우·위·으·이 | 있다 |
| 목업 `nav__jamo__resume` | **10자** — ㅏㅓㅗㅜㅡㅣ**ㅐㅔ**ㅚㅟ | 있다 |
| 앱 `unit.ts` 의 `title` | **8자** — ㅏㅓㅗㅜㅡㅣㅚㅟ | **없다** |

원장과 목업이 같고 **앱만 둘이 빠졌다.** 앱의 `unit.title` 은 구 콘텐츠에서 온 것이라
포팅 대상인데, 그 값이 화면에 그대로 나오고 있다. 검수 때 이 단원 제목들을
원장 기준으로 맞춰야 한다.

그래서 자모 목록은 **목업 대조에 아직 넣지 않았다.** 넣으면 이 차이로 계속 붉게
나오는데 지금은 고칠 수 없다(검수 대기). 검수가 끝나면
`app/scripts/activity-parity.tsx` 에 한 줄(`SCREENS.nav__jamo__resume = <Jamo />`)로
들어간다 — 그 화면은 컴포넌트를 그대로 그릴 수 있다는 것을 확인해 두었다.
