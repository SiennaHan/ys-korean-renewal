...
## 8. 문법 빈칸 해설 · 다섯 건 고쳤다 (2026-08-21)

`n4_blank_question` 의 `grammar_focus_수정`(앱에서는 `grammar_focus_revised`)이
**학습자용 해설**이다. 836행 전부 채워져 있고 평균 34자다 —
"받침이 있어요. 먹+을까요" 처럼. API 응답에 담지 않고 문항 조회로 받는다
(`api_schema_v1` §6 의 1번).

그것을 확인하다가 **해설이 그 문항의 것이 아닌 다섯 건**을 찾아 고쳤다.
저작 중 복사한 뒤 해설을 안 고친 것으로 보인다.

| 급·과 | 문항 | 정답 | 고치기 전 해설 |
|---|---|---|---|
| 1급 6과 | `가방 안에 지우개( ) 있어요.` | `가` | `연필'은 받침이 있어요. 연필+이` |
| 1급 8과 | `형이 사진( ) 봐요.` | `을` | `영화'는 받침이 없어요. 영화+를` |
| 1급 10과 | `책을 읽( )?` | `-을까요` | `받침이 없어요. 마시+ㄹ까=마실까요` |
| 1급 13과 | `방학에 운동( ) 배우고 싶어요.` | `을` | `한국어'는 받침이 없어요. '를'을 사용해요.` |
| 4급 9과 | `부모님이 제 이름을 직접 ( ).` | `지으셨어요` | `'짓다'는 '-으니까/니까'를 만나면 '지으니까'` |

**앞의 넷이 특히 나빴다** — 받침 유무로 조사·어미를 고르는 문항인데 해설이
**정답과 반대되는 쪽**을 설명했다. 학습자가 해설을 읽으면 틀린 규칙을 배운다.

4급 9과는 종류가 다르다. 정답 `지으셨어요` 는 ㅅ불규칙에 **높임 `-으시-` 와
과거 `-었어요` 가 겹친 것**인데 해설은 `-으니까` 를 설명하고 있었다(같은 과의
`병이 다 나으니까` 에서 옮겨온 것으로 보인다). 그 겹침이 보이게 다시 썼다 —
"ㅅ동사 '짓다'는 높임 '-으시-'를 만나면 'ㅅ'이 탈락해 '지으시다'가 되고,
과거 '-었어요'가 붙어 '지으셨어요'가 됩니다."

**고친 곳** — 원장 `글로벌_교재기반_콘텐츠_v24.xlsx` 의 `n4_blank_question`.
`grammar_focus` 와 `grammar_focus_수정` 둘 다 고치고 `change_note` 에 사유를 남겼다.
`build-content.py` 로 앱에 반영했다(바뀐 시트는 이것 하나, 앱 JSON 15줄).
원장 대조: 시트 37개 그대로, 달라진 행 정확히 5.

### 남은 검수 — 자동으로는 여기까지다

같은 검사를 다시 돌려 **"반대 이형태" 는 0건**임을 확인했다. 다만 자동 검사에는
한계가 있다. "해설이 인용한 낱말이 문항에 없다" 로 걸면 27건이 나오는데
**대부분 정상**이다 — 해설이 **기본형**을 인용하는 것(`마시다` · `가다` · `듣다`)과
문법 자리를 가리키는 말(`장소+에서`)이 그렇다. 옳은 교수법이다.

**836행 전수 검수는 사람이 해야 한다.** 기계가 잡을 수 있는 것은
"정답과 반대되는 이형태" 처럼 규칙으로 쓸 수 있는 종류뿐이고, 그건 이제 0이다.

### 여는 따옴표 61개 복원 — 엑셀이 먹은 것이었다

해설이 `연필'은 받침이…` 처럼 **여는 따옴표 없이** 적혀 있었다. 원인은 저작 실수가
아니라 **엑셀이다** — 셀 맨 앞의 `'` 는 "이건 텍스트다" 라는 서식 지시로 해석돼
값에 남지 않는다. 저작자는 `'연필'은` 이라고 제대로 썼는데 입력 시점에 먹힌 것이다.

원장 전체를 훑어 따옴표가 홀수인 셀 406개를 찾았고, 그 중 **실제 대상은
`n4_blank_question` 의 `grammar_focus_수정` 61개**였다. 나머지는 영어
아포스트로피(`I'm` · `o'clock` · `one's`)라 정상이고, `99_변경내역_*` 은 이력이라
건드리지 않았다.

| | |
|---|---|
| 여는 따옴표 복원 | 60개 — 맨 앞에 `'` 를 되돌렸다 |
| 군더더기 제거 | 1개 — `때 ' '-어서/…` 의 `' ` 를 없앴다(여는 따옴표가 아니라 오타) |
| 고친 뒤 홀수 | **0개** |

**⚠️ 엑셀에서 그 셀을 다시 타이핑하면 또 먹힌다.** 지금은 openpyxl 이 값을 그대로
써 넣어 살아 있지만, 사람이 엑셀에서 그 칸을 고쳐 저장하면 맨 앞 `'` 가 다시
서식 지시로 해석된다.

**그래서 생성기가 매번 검사한다.** `build-content.py` 가 따옴표 홀수인 **한글** 칸을
찾아 시트·행·칸 이름과 함께 경고한다. 영어 아포스트로피(`I'm` · `o'clock`)는
정상이므로 한글이 절반을 넘는 칸만 센다. 지금은 0건이고, 일부러 되돌려 보면
그 행을 정확히 짚는 것까지 확인했다.
