...
**같이 볼 것 둘 — 아직 안 바꿨다.**
* `write3`(sub=6)이 아직 `받침까지 조합하고 쓰기` 다. 같은 규칙이면
  `받침까지 조합하기` 가 맞을 텐데 지시받은 것이 아니라 그대로 뒀다
* `module.ts` 의 `eng` 가 아직 `Consonant-Vowel writing` 이다. 이 값은
  리뉴얼 화면이 아니라 구 `routes/book/*` 이 쓴다
...
| `n8_jamo` | **포팅 완료**(v30 · 529행). 배관도 끝났다 — §2 |
...
**`npx biome check src` 는 아직 통과하지 않는다 — 505 → 220 건까지 줄였다 (2026-08-24).**
