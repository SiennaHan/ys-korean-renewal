...
들어갔다** — 그때 스물일곱, 지금은 **47** 이다(VocaShot 셋과 게임 열일곱이 더 들어왔다). 렌더 뒤에 붙는 스크롤 표시 속성(`data-cue-bound` 등)은
디자인이 아니라 걷어냈다.
...
**사실은 이랬다.** 커밋 `d1a7cfb` 가 답을 갖고 있었다 — 캡처 47개는 전부
...
### 캡처 47개가 다 들어왔다 — 대조가 47화면 (2026-08-24)
