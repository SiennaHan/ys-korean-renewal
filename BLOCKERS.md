...
제외가 실제로 동작하게 되면서 `app/src/routeTree.gen.ts` 가 두 판으로 갈린다.
이 파일은 **생성물인데 저장소에 추적된다**(`main.tsx` 가 import 하고,
`pnpm build` 가 `tsc` 를 먼저 돌려서 새 클론에도 파일이 있어야 한다).

| | 줄 | 테스트 라우트 |
|---|---|---|
| 개발 판 — **이것을 커밋한다** | 1,463 | 있다 (`/test/**` 로 디버깅) |
| 프로덕션 판 — `pnpm build` 가 남긴다 | 1,274 | 없다 |

처음에는 **프로덕션 판을 커밋하면 `pnpm typecheck` 가 깨졌다** —
`routes/test/*.tsx` 가 트리에 없는 라우트 id 를 참조하기 때문이다(실측했다).
`app/tsconfig.json` 의 `exclude` 에 `src/routes/test` 를 넣어 **어느 판을 커밋해도
통과하게** 했다. 대가는 그 아홉 개가 타입 검사를 받지 않는 것이고, 벤더 이관
(2026-08-19) 뒤 아무도 손대지 않은 디버그 페이지라 감수했다.

남은 것은 **지저분함뿐**이다 — 릴리스 빌드 뒤 이 파일이 수정된 것으로 보인다.
되돌리려면 `git checkout app/src/routeTree.gen.ts`. 커밋해 버려도 **깨지지는 않고**,
다음에 `pnpm dev` 를 돌리면 개발 판으로 스스로 돌아온다.

> **근본적으로 없애려면 `src/routes/test/` 아홉 개를 지우면 된다** — 그러면 트리가
> 환경과 무관해지고 갈림 자체가 사라진다. 1,196줄 + JSON 셋이고 **벤더 이관 커밋
> 이후 아무도 손대지 않았다**(전부 `2026-08-19`). 지금 깨진 곳을 참조하지도 않는다.
> 쓸모가 남았는지는 이 저장소로 판단할 수 없어 남겨 두었다.
