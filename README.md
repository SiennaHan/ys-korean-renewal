...
학생용 앱을 리뉴얼하는 작업 저장소다. 문서가 여럿인데 **처음에 볼 것은 셋뿐이다.**
...
구조를 비교한다. **49개 화면이 일치한다** — 활동 22 + 내비 5(홈 셋 · 교재학습 목록 ·
자모 목록) + VocaShot 5(시작 · 플레이 둘 · 결과 둘) + 게임 17(목록 하나 + 게임 넷 × 화면 넷).
캡처 49개가 곧 목업 전부다 — 남은 것이 없다.
