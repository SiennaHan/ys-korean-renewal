...
학생용 앱을 리뉴얼하는 작업 저장소다. 문서가 15개인데 **처음에 볼 것은 셋뿐이다.**
...
| `phase1/` | 기획·명세·목업 HTML 15개 + `_superseded/` 39개.<br>그 밖에 인계 메모 셋(`*_handoff_note.txt`) · 디자인 토큰(`tokens.css` · `figma_*.json`) · 가짜 API(`game_mockapi.py`) |
...
비슷하지만 **다른 지표 셋이 공존한다** — 이식 화면 24(`handoff_v2` §02 표 합계) ·
