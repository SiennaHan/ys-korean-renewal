...
| `글로벌_교재기반_콘텐츠_v*.xlsx` | **콘텐츠 원장 정본 — 저장소 루트의 가장 높은 번호다.** 번호를 적지 마라.<br>교재 파생이라 `.gitignore` 가 막는다. 옛 판은 `_ledger_superseded/` 에 있다 — 지우지 마라(§아래).<br>앱 JSON 은 자모·미션대화를 아직 안 받는다 — 미션대화는 **검수 완료**·생성기 배선 대기, 자모는 검수 대기 — `BLOCKERS.md` §2 · §8 |
