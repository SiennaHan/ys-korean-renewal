...
`check_docs.py` 가 **정본 26개가 모두 색인에 있는지** 검사한다.
...
| `phase1/` | 기획·명세·목업 HTML 26개 + `_superseded/` 22개.<br>그 밖에 인계 메모 셋(`*_handoff_note.txt`) · 디자인 토큰(`tokens.css` · `figma_*.json`) · 가짜 API(`game_mockapi.py`) |
...
이 저장소는 문서를 **이름과 절 번호로** 인용한다(절 인용이 408개다). 그래서 한 곳을
...
"남은 일" 로 앉아 있었다. 지금 세는 것은 다섯이다.
    ...
    절 인용 § 총합 (본문 기준 — CSS·JS 안의 § 는 안 센다)
    인계 메모 수
