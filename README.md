...
구조를 비교한다. **23개 화면이 일치한다** — 활동 22 + 교재학습 목록 1.
봐주는 차이는 실행할 때마다 같이 찍히므로 무엇을 눈감아 주는지 숨지 않는다.

홈·자모 목록은 아직 들어가 있지 않고 이유가 서로 다르다. **홈**은 데이터를 넣을 길이
없어 정적으로 그리면 스피너만 나온다 — 표시와 데이터를 갈라야 한다. **자모**는 그려지는데
목업과 내용이 달라서 보류했다(모음1 이 목업·원장은 10자, 앱은 8자) — 자모 콘텐츠가
검수 대기이므로 [BLOCKERS.md](BLOCKERS.md) §2 에 검수 항목으로 적어 두었다.
