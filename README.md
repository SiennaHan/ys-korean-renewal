...
`check_docs.py` 가 **정본 25개가 모두 색인에 있는지** 검사한다.
...
| `phase1/` | 기획·명세·목업 HTML 25개 + `_superseded/` 23개.<br>그 밖에 인계 메모 셋(`*_handoff_note.txt`) · 디자인 토큰(`tokens.css` · `figma_*.json`) · 가짜 API(`game_mockapi.py`) |
