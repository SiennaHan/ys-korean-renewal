...
학생용 앱을 리뉴얼하는 작업 저장소다. 문서가 21개인데 **처음에 볼 것은 셋뿐이다.**
...
`check_docs.py` 가 **정본 21개가 모두 색인에 있는지** 검사한다.

색인에는 이런 것이 있다 — 계획·결정 셋("기획서" 라 부를 것들이 어떻게 다른가) ·
시각 정본 목업 셋 · **셸 명세 넷이 어느 층인가**(행동·구조·값·순서) ·
...
| `phase1/` | 기획·명세·목업 HTML 21개 + `_superseded/` 30개.<br>그 밖에 인계 메모 셋(`*_handoff_note.txt`) · 디자인 토큰(`tokens.css` · `figma_*.json`) · 가짜 API(`game_mockapi.py`) |
