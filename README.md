...
| `글로벌_교재기반_콘텐츠_v*.xlsx` | **콘텐츠 원장 정본 — 저장소 루트의 가장 높은 번호다.** 번호를 적지 마라.<br>교재 파생이라 `.gitignore` 가 막는다. 옛 판은 `_ledger_superseded/` 에 있다 — 지우지 마라(§아래).<br>앱 JSON 은 **미션대화의 생성기 배선만** 남았다 — 자모는 배선까지 끝났다(2026-08-24, 화면 여섯이 `n8_jamo` 를 읽는다).<br>**자모는 검수도 끝났다(2026-08-28)** — 529행 전부 `reviewed` 다. `BLOCKERS.md` §2 · §2-c · §8 |
...
**종류는 여기 나열하지 않는다.** 전에 15개를 표로 옮겨 적어 뒀는데 검사가 18종이 되면서
**`앵커` · `사실 중복` · `색인` 셋이 빠진 채로 남았다** — 그중 `사실 중복` 은 이 저장소가
가장 많이 어긴 규칙("같은 수를 두 문서에 적지 마라")을 지키는 검사다.
목록은 `check_docs.py` 의 머리말이 쥐고, 개수는 돌 때 스스로 센다.
