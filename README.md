...
| `글로벌_교재기반_콘텐츠_v*.xlsx` | **콘텐츠 원장 정본 — 저장소 루트의 가장 높은 번호다.** 번호를 적지 마라.<br>교재 파생이라 `.gitignore` 가 막는다. 옛 판은 `_ledger_superseded/` 에 있다 — 지우지 마라(§아래).<br>앱 JSON 은 **미션대화의 생성기 배선만** 남았다 — 자모는 배선까지 끝났다(2026-08-24, 화면 여섯이 `n8_jamo` 를 읽는다).<br>자모에 남은 검수는 받침·겹받침 낱자 목록뿐이고 배관을 막지 않는다 — `BLOCKERS.md` §2 · §2-c · §8 |
...
검사 종류는 여기 적지 않는다 — `check_docs.py` 가 돌 때 스스로 세어 찍는다.
**전에 이 줄이 "여섯", 검사기 주석이 "일곱", 실제 구현이 여덟이었다.**
...
| 6 | 화면 승격 | `phase1/_snapshots/`(시점 기록)와 `app/src/screens_ref/`(대조 기준)가 갈라졌는데<br>`phase1/screen_promotions.md` 표에 줄이 없는 곳 |
...
| 10 | **하위절** | `§N.M` 인데 그 번호의 h3 이 없는 곳 |
| 11 | **하위절 중복** | 한 문서에 같은 `N.M` 이 둘 이상 — 합치면 생긴다 |
| 12 | **원장 버전** | 문서가 원장 정본 버전을 못박았는데 지금 것과 다른 곳.<br>산문("정본은 vN")과 파일명(`…_vN.xlsx`) 둘 다 본다 — 시점 기록의 옛 판 언급은 안 잡는다 |
| 13 | **데이터 정본** | 문서가 말하는 데이터 출처와 코드의 `import` 가 다른 곳 |
| 14 | **문구 지뢰** | 한 번 거짓이라 밝혀진 **표현**이 남아 있거나 되살아난 곳 — `STALE_PHRASES`.<br>숫자가 아니라 문장이라 5번이 못 잡는 것을 본다 — 플래시카드가 1급 전용이라던 문장이 문서 다섯에 남았던 일.<br>일부러 남기는 곳은 `allow` 에 이유를 적는다. **아무것도 막지 않는 `allow` 도 잡는다**.<br>⚠️ 인용과 주장을 가리지 못한다 — 문서에서 그 표현을 **예시로 옮겨 적으면 그것도 걸린다**. 풀어 써라 |
| 15 | **관찰 기준** | 문서가 `<!-- 관찰: 경로 @ 커밋 -->` 로 선언한 경로가 **그 커밋 이후 바뀐 곳**.<br>어느 문장이 틀렸는지는 말하지 않는다 — "이 문서를 다시 봐라" 만 말한다.<br>다시 보고 고칠 것이 없으면 **기준 커밋만 지금으로 올린다**. 경로는 좁게 골라라 — 자주 우는 검사는 안 보고 넘긴다 |
...
비슷하지만 **다른 지표 셋이 공존한다** — 이식 화면 25(`masterplan_v3` §15 표 합계) ·
