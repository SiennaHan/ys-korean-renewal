...
| `글로벌_교재기반_콘텐츠_v28.xlsx` | **콘텐츠 원장 정본.** 교재 파생이라 `.gitignore` 가 막는다.<br>앱 JSON 은 자모·미션대화를 아직 안 받는다(검수 대기) — `BLOCKERS.md` §2 · §2-b |
...
구조를 비교한다. **30개 화면이 일치한다** — 활동 22 + 내비 5(홈 셋 · 교재학습 목록 · 자모 목록) + VocaShot 셋(시작 · 플레이 · 결과).
