...
- `DEV-04` 월 구독 결제와 실제 권한 부여 · P0 ← **막은 결정** PD-01 결제 상품
...
- `DEV-17` 축 점수를 「전부-아니면-전무」에서 오류 밀도로 · P1 (기획 확정 · 코치 출력 변경 필요)
- `DEV-18` 대화 AI 가 이미 끝난 미션을 다시 묻는다 · P1 (원인 셋 확인 · 전부 서버 안)
- `DEV-19` 만료 시각의 시간대가 9시간 어긋난다 · P1 (발견만 · 세 곳을 같이 봐야 한다)
- `DEV-20` 정의되지 않은 CSS 변수 둘 · P2 (발견만 · 어떤 게이트도 안 잡는다)
