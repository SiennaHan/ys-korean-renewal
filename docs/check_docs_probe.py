...
CASES: list[tuple] = [
    ...
    #
    # 「이식 화면 수 (산문 꼴)」 항목이 여기 있었다. **2026-09-03 에 그 검사를 지웠다** —
    # 수를 `status.generated.md` 가 내게 하고 문서에서 걷었으므로 대조할 손글씨가 없다
    # (`doc_review_v1.md` §6-b). 검사가 없어지면 그것을 지키던 대조군도 없애야 한다 —
    # 남겨 두면 **「안 운다」로 영영 빨간불**이고, 그러면 하네스 자체를 아무도 안 돌린다.
    ("숫자 주장 — 페이월 상태 수 (한글 수사 「넷」)", "숫자 주장",
     replace("docs/paywall_SOT.html", "다섯의 다음 행동", "넷의 다음 행동")),
    # **「라인업 8종」과 갈리는지가 요점이다.** 둘 다 참인 다른 단위라, 패턴이 넓으면
    # masterplan · dev_spec · shell_spec · G1 의 열 곳을 잘못 잡는다.
    ("숫자 주장 — 과 활동 종 수", "숫자 주장",
     replace("README.md", "활동 일곱 종으로", "활동 여섯 종으로")),
    ("과 활동 종 수 — 라인업 8종은 안 건드린다 (안 울어야 정상)", "",
     append("README.md", "\n셸에 활동 8종을 이식했다. 라인업 8종 중 3종 시트가 없다.\n")),
    ...
    ("상태 계약 — 콘텐츠 공개 제외값을 deleted 아닌 값으로 바꾼다", "상태 계약",
     replace("docs/project_status.json", '"excluded_review_statuses": ["deleted"]',
             '"excluded_review_statuses": ["draft"]')),
    ("상태 계약 — 모르는 review_status 를 버린다고 주장", "상태 계약",
     replace("docs/project_status.json", '"unknown_status_behavior": "warn_and_include"',
             '"unknown_status_behavior": "drop"')),
    ("상태 계약 — 생성기의 제외값이 정책과 다르다", "상태 계약",
     replace("app/scripts/build-content.py", 'DROP_STATUS = {"deleted"}',
             'DROP_STATUS = {"draft"}')),

    ("숫자 주장 — 원장 버전", "원장 버전",
     append("README.md", "\n원장 정본은 v3 이다.\n"), has_ledger),
    # **주인을 갈아 끼웠다(2026-09-03).** 전에는 「이식 화면 수」로 시험했는데 그 검사를
    # 지웠다. `사실 중복` 자체는 살아 있으므로(OWNER 일곱) 남은 주인 중 하나로 바꾼다 —
    # 페이월 상태 수의 주인은 `paywall_SOT` 다.
    ("사실 중복 — 주인이 아닌 문서가 그 수를 적는다", "사실 중복",
     append("docs/user_flow_v1.html", "<p>페이월이 다섯 상태로 갈린다.</p>\n")),
