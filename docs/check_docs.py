...
STALE_PHRASES: list[tuple[str, str, dict[str, str]]] = [
    ...
    (
        ...
        r"|가격[^\n]{0,6}(?:·|,)[^\n]{0,6}PG[^\n]{0,6}미정",
        "가격은 월 11,000원(부가세 포함 · KRW 단일)이다 — 기획 2026-09-15 "
        "(access_and_pricing_v1 §07). 월 6달러(2026-08-28) → 7,900원(2026-09-04) → 11,000원 "
        "으로 두 번 바뀌었다. 「약 8달러」는 참고 금액이고 청구 통화가 아니다. 남은 것은 "
        "무료 체험·환불·해지 시점·실패·유예 정책이지 '가격'·'통화'·'세금' 이 아니다",
        {
    ...
    (
        r"(?:가격은|월 가격은|금액은|구독료는|가격이)[^\n]{0,20}7,900원",
        "가격은 2026-09-15 에 월 11,000원으로 바뀌었다 (PD-01 · access_and_pricing_v1 §07). "
        "7,900원은 2026-09-04 부터 그날까지의 값이다 — 지나간 값으로 적는 것은 괜찮지만 "
        "'지금 가격' 으로 적으면 안 된다. 숫자의 주인은 app/src/shared/constants 의 "
        "MONTHLY_PRICE_KRW 하나이고 화면·정본은 거기서 읽는다",
        {
            "doc_review_v1": "지뢰 안내문이 낡은 사례를 적은 자리다 — 2026-09-04 에 "
            "6달러에서 7,900원으로 바뀐 것을 기록한 관찰 스탬프이고, 지금 가격 주장이 아니다",
        },
    ),
    (
        r"2~8급(?:은|에는|을)[^\n]{0,24}(?:플래시카드[^\n]{0,12}없|늘 잠겨|늘 <?span?[^\n]{0,20}off)",
