...
## 감사 뒤 제품 반영 — 1~5단계 (2026-08-25)

이 절만 **현재 제품 상태**를 말한다. 아래의 `MATCH`·`DRIFT` 표는 당시 문제를 다시
추적할 수 있도록 고치지 않았다.

| 범위 | 현재 반영 |
|---|---|
| 화면 변형·진행 표시 | 자모 듣기 2/4지선다, 읽기 4지선다/OX를 정본 점검 화면에서 모두 볼 수 있다. 2~15개는 분절형, 16개 이상은 연속형+n/N, 1개와 내부 학습목록형은 진행 표시를 생략한다. **어휘 미리보기는 숨기고 분모에서 제외하며, 첫 어휘 문제를 1/N로 계산한다.** 2~15개일 때는 숫자 없이 칸으로만 보인다. 자모 조합 실제 라우트에도 진행 표시를 연결했다 |
| 선택지 채점 | `ChipOption`과 문법 빈칸도 `Choice` 규칙으로 통일했다. 오답은 고른 항목만 표시하고 정답은 공개하지 않으며, **2초 뒤 선택지·문장 반영·피드백 pill을 함께 거둔다** |
| 롤플레잉 | 실제 `ai-roleplay`가 확정 셸과 `RoleplayLayout`을 사용한다. 기존 TTS·녹음·평가 로직은 유지했다 |
| 녹음 | `idle → preparing(2초) → recording → finishing(1초) → done → sending` 하나로 통일했다. 준비·전송은 spinner, 완료만 ✓를 쓴다 |
| 플래시카드 | 실제 재생·결과 화면을 공통 셸로 옮겼다. 앞/뒷면 어느 쪽에서도 「몰라요/알아요」가 활성이고, 27장을 끝내면 현재 세션 판정으로 결과를 계산한다 |
| 공통 음성 아이콘 | 몸체와 파형 끝이 모두 둥근 스피커로 실제 컴포넌트·목업 정본을 함께 교체했다 |

`npm run parity:activity`의 정본 49화면은 이 반영 뒤 **전부 일치**한다. 미션대화의
진행 화면 전체 이관, 결과·쓰기 모달·나가기 예외는 다음 단계다.
...
## 녹음 상태 머신 불일치 — 감사 당시 기록

위 「감사 뒤 제품 반영」에서 해결했다. 아래는 통합 전 원인을 남긴 기록이다.
...
3. `Choice`/`ChipOption` 채점 규칙 통일 ← **했다**
4. 녹음 상태 머신과 아이콘 통일 ← **했다**
5. 실제 롤플레잉·플래시카드를 확정 컴포넌트 구조로 옮김 ← **했다**
   · 미션대화 진행 화면 전체 이관은 남음
